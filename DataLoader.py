...
class DataModule(pl.LightningDataModule):
    ...
    def parse_one_database(self, data_path):
        X, y = [], []
        labels_csv = pd.read_csv(os.path.join(data_path, 'labels.csv'))

        for i, rec in enumerate(tqdm(os.listdir(os.path.join(data_path, 'recordings')))):
            ...
            if (self.short_sample is True) and (i == 5):
                break
    ...
    def setup(self, stage=None):
        if stage == TrainerFn.FITTING:
            X, y = self.parse_one_database(self.kaggle_path)
